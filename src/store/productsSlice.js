...
export const productsSlice = createSlice({
  ...
  reducers: {
    ...
    detailsProduct: (state, action) => state.map(product => 
      product.id === action.payload.id ? action.payload : product),
  }
});

export const { setProducts, addProduct, updateProduct, deleteProduct, detailsProduct } = productsSlice.actions;
...
export const detailProduct = (productId) => ({ type: `product/${productId}`, payload: { id: productId } });
